        ...
        <div class="d-flex flex-column flex-row-fluid">
            ...
            <ul class="breadcrumb breadcrumb-separatorless fw-semibold mb-3 fs-7">
                ...
                <li class="breadcrumb-item text-gray-700">
                    <a href="{{ route('prt.apps.page.index', [$tags]) }}" class="text-gray-700 text-hover-primary">{{ $kategori }}</a>
                </li>
                {{-- end::Item --}}
                {{-- begin::Item --}}
                <li class="breadcrumb-item">
...
@section('content')
    {{-- begin::Form --}}
    <form id="kt_halaman_form" class="form d-flex flex-column flex-lg-row"
        action="{{ isset($data) ? route('prt.apps.page.update', [$tags, $uuid_enc]) : route('prt.apps.page.store', [$tags]) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @isset($data)
            @method('PUT')
        @endisset
            ...
            <div class="card card-flush py-4">
                ...
                <div class="card-body text-center pt-0">
                    ...
                    <select class="form-select mb-2 @error('status') is-invalid @enderror" data-control="select2" data-hide-search="true" data-placeholder="Pilih Status"
                        id="kt_halaman_status" name="status" required>
                        <option></option>
                    ...
                    @enderror
                    {{-- begin::Description --}}
                    <div class="text-muted fs-7">Pilih status publikasi halaman.</div>
                    {{-- end::Description --}}
                </div>
                {{-- end::Card body --}}
            </div>
                ...
                <div class="card-body pt-0">
                    ...
                    <div class="d-flex flex-column gap-5">
                        ...
                        <div class="m-0 p-0">
                            ...
                            <span class="text-gray-800 fw-bold" id="halaman_slug_preview">-</span>
                        </div>
                        <div class="m-0 p-0">
        ...
        <div class="d-flex flex-column flex-row-fluid gap-7 gap-lg-10">
                ...
                <div class="card-body pt-0">
                    ...
                    <div class="mb-10 fv-row">
                        ...
                        <input type="number" name="no_urut" class="form-control mb-2 @error('no_urut') is-invalid @enderror" placeholder="Masukkan nomor urut"
                            value="{{ old('no_urut', isset($data) ? $data->no_urut : \Helper::GetNoUrutHalaman($tags)) }}" autocomplete="off" min="1" required />
                        {{-- end::Input --}}
                        {{-- begin::Description --}}
                        <div class="text-muted fs-7">Nomor urut untuk pengurutan halaman.</div>
                        {{-- end::Description --}}
                        @error('no_urut')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="mb-10 fv-row">
                        ...
                        {{-- end::Label --}}
                        {{-- begin::Input --}}
                        <input type="text" name="judul" id="judul" class="form-control mb-2 @error('judul') is-invalid @enderror" placeholder="Masukkan judul halaman"
                            value="{{ old('judul', isset($data) ? $data->judul : '') }}" maxlength="300" autocomplete="off" required />
                        {{-- end::Input --}}
                        {{-- begin::Description --}}
                        <div class="text-muted fs-7">Judul halaman maksimal 300 karakter.</div>
                        {{-- end::Description --}}
                        @error('judul')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="mb-10 fv-row">
                        ...
                        {{-- end::Input --}}
                        {{-- begin::Description --}}
                        <div class="text-muted fs-7">Tanggal dan waktu publikasi halaman.</div>
                        {{-- end::Description --}}
                        @error('tanggal')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
            ...
            <div class="card card-flush py-4">
                ...
                <div class="card-body pt-0">
                    ...
                    <div class="fv-row">
                        {{-- begin::Label --}}
                        <label class="required form-label">Isi Halaman</label>
                        {{-- end::Label --}}
                        {{-- begin::Editor --}}
                        <textarea name="post" id="post" class="form-control @error('post') is-invalid @enderror" placeholder="Masukkan konten halaman" required>{{ old('post') }}</textarea>
                        {{-- end::Editor --}}
                        @error('post')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
            ...
            <div class="d-flex justify-content-end">
                {{-- begin::Button --}}
                <a href="{{ route('prt.apps.page.index', [$tags]) }}" id="kt_halaman_cancel" class="btn btn-light me-5">
                    <i class="ki-outline ki-arrow-left fs-2"></i>Batal
                </a>
                {{-- end::Button --}}
                {{-- begin::Button --}}
                <button type="submit" id="kt_halaman_submit" class="btn btn-primary">
    ...
    <script>
        ...
        var KTAppHalamanSave = function() {
            ...
            var handleForm = function(e) {
                ...
                submitButton.addEventListener('click', function(e) {
                    ...
                    if (!judul) {
                        isValid = false;
                        errorMessage = 'Judul halaman wajib diisi';
                    } else if (judul.length > 300) {
                        isValid = false;
                        errorMessage = 'Judul halaman maksimal 300 karakter';
                    }
                    ...
                    if (!post || post === '<p></p>' || post === '' || post === '<p><br></p>') {
                        isValid = false;
                        errorMessage = 'Konten halaman wajib diisi';
                    }
            ...
            var handleSlugPreview = function() {
                ...
                const slugPreview = document.querySelector('#halaman_slug_preview');
        ...
        `,
            ...
            return {
                init: function() {
                    // Elements
                    form = document.querySelector('#kt_halaman_form');
                    submitButton = document.querySelector('#kt_halaman_submit');
                    cancelButton = document.querySelector('#kt_halaman_cancel');
