<?php
namespace App\Http\Controllers\web\backend\emagazine;
...
class EmagazineController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // cek filter
        if ($request->session()->exists('filter_status_emagazine')) {
            $status = $request->session()->get('filter_status_emagazine');
        } else {
            $request->session()->put('filter_status_emagazine', 'Published');
            $status = "Published";
        }
        ...
        if ($request->ajax()) {
            if (isset($_GET['filter'])) {
                ...
                $request->session()->put('filter_status_emagazine', $status);
            } else {
                $status = $request->session()->get('filter_status_emagazine');
            }
            // cek role
            if ($role == "Super Admin" || $role == "Admin" || $role == "Editor") {
                if ($status == "Draft") {
                    $data = PortalEmagazine::whereStatus($status)
            ...
            return Datatables::of($data)
                ...
                ->addColumn('judul', function ($data) {
                    ...
                    $edit       = route('prt.apps.emagazine.edit', $uuid_enc);
                    $judul      = '
        ...
        return view('pages.admin.portal_apps.emagazine.index', compact(
            'status'
        ));
    }
    ...
    public function create()
    {
        ...
        $auth = Auth::user();
        // get kategori
        $kategoriList = PortalKategori::whereType("Emagazine")->whereStatus("1")->orderBy("nama")->get();
        $title        = "Tambah Data Emagazine";
        $submit       = "Simpan";
        return view('pages.admin.portal_apps.emagazine.create_edit', compact(
            'title',
            'submit',
            'auth',
            'kategoriList'
    ...
    public function store(Request $request)
    {
        ...
        $request->validate([
            ...
            "tanggal"        => "required",
            'file_emagazine' => 'required|file|mimes:pdf|max:204800', // 200 MB
        ]);
        ...
        $cekslug   = PortalEmagazine::whereSlug($slug)->count();
        $inputslug = $cekslug > 0 ? $slug . "-" . Helper::gencode(4) : $slug;

        // value
        $uuid    = Str::uuid();
        $path    = "emagazine/" . date('Y') . "/" . $uuid;
        ...
        // file_emagazine
        if ($request->hasFile('file_emagazine')) {
            $img = Helper::UpFilePdf($request, "file_emagazine", $path);
            if ($img == "0") {
                alert()->error('Error!', 'Gagal Menyimpan Data, File Emagazine Tidak Sesuai Format!');
                return back();
            }
            $value_1['url']  = $img['url'];
        ...
        if ($request->hasFile('thumbnails')) {
            $img = Helper::UpThumbnailsEmagazine($request, "thumbnails", $path);
            if ($img == "0") {
        ...
        // save
        $save_1 = PortalEmagazine::create($value_1);
        if ($save_1) {
            // create log
            $aktifitas = [
                "tabel" => ["portal_emagazine"],
                "uuid"  => [$uuid],
                "value" => [$request->judul],
            ];
            $log = [
                "apps"      => "Portal Apps",
                "subjek"    => "Menambahkan Data Emagazine UUID= " . $uuid,
                "aktifitas" => $aktifitas,
                "device"    => "web",
            ];
            Helper::addToLogAktifitas($request, $log);
            alert()->success('Success', "Berhasil Menambahkan Data!");
            return redirect()->route('prt.apps.emagazine.index');
        } else {
    ...
    public function edit($uuid_enc)
    {
        ...
        $data = PortalEmagazine::findOrFail($uuid);
        // get kategori
        $kategoriList = PortalKategori::whereType("Emagazine")->whereStatus("1")->orderBy("nama")->get();
        $title        = "Edit Data Emagazine";
        $submit       = "Simpan";
        return view('pages.admin.portal_apps.emagazine.create_edit', compact(
            'uuid_enc',
            'title',
    ...
    public function update(Request $request, $uuid_enc)
    {
        ...
        $request->validate([
            ...
            "tanggal"        => "required",
            'file_emagazine' => 'sometimes|nullable|file|mimes:pdf|max:204800', // 200 MB
        ]);
        ...
        if ($data->judul !== $request->judul) {
            $slug      = \Str::slug($request->judul);
            $cekslug   = PortalEmagazine::where('uuid', '!=', $uuid)->whereSlug($slug)->count();
            $inputslug = $cekslug > 0 ? $slug . "-" . Helper::gencode(4) : $slug;
        } else {
            $inputslug = $data->slug;
        }
        ...
        if ($save_1) {
            ...
            $log = [
                "apps"      => "Portal Apps",
                "subjek"    => "Mengubah Data Emagazine UUID= " . $uuid,
                "aktifitas" => $aktifitas,
                "device"    => "web",
            ];
            Helper::addToLogAktifitas($request, $log);
    ...
    public function destroy(Request $request)
    {
        ...
        if ($data->status == "Draft" || $data->status == "Pending Review") {
            ...
            $path  = "emagazine/{$tahun}/{$data->uuid}";
        ...
        if ($save_1) {
            ...
            $log = [
                "apps"      => "Portal Apps",
                "subjek"    => "Menghapus Data Emagazine UUID= " . $uuid,
                "aktifitas" => $aktifitas,
                "device"    => "web",
            ];
            Helper::addToLogAktifitas($request, $log);
