<?php
namespace App\Http\Controllers\web\backend\infografis;
...
class InfografisController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // cek filter
        if ($request->session()->exists('filter_status_infografis')) {
            $status = $request->session()->get('filter_status_infografis');
        } else {
            $request->session()->put('filter_status_infografis', 'Published');
            $status = "Published";
        }
        ...
        if ($request->ajax()) {
            if (isset($_GET['filter'])) {
                ...
                $request->session()->put('filter_status_infografis', $status);
            } else {
                $status = $request->session()->get('filter_status_infografis');
            }
            // cek role
            if ($role == "Super Admin" || $role == "Admin" || $role == "Editor") {
                if ($status == "Draft") {
                    $data = PortalInfografis::whereStatus($status)
            ...
            return Datatables::of($data)
                ...
                ->addColumn('judul', function ($data) {
                    $thumbnails = Helper::thumbnail($data->url);
                    $uuid_enc   = Helper::encode($data->uuid);
                    $edit       = route('prt.apps.infografis.edit', $uuid_enc);
                    $judul      = '
        ...
        return view('pages.admin.portal_apps.infografis.index', compact(
            'status'
        ));
    }
    ...
    public function create()
    {
        ...
        $auth = Auth::user();
        // get kategori
        $kategoriList = PortalKategori::whereType("Infografis")->whereStatus("1")->orderBy("nama")->get();
        $title        = "Tambah Data Infografis";
        $submit       = "Simpan";
        return view('pages.admin.portal_apps.infografis.create_edit', compact(
            'title',
            'submit',
            'auth',
            'kategoriList'
    ...
    public function store(Request $request)
    {
        ...
        $request->validate([
            ...
            "kategori"        => "required",
            "file_infografis" => "required|image|mimes:png,jpg,jpeg|max:2048",
            "tanggal"         => "required",
        ]);
        ...
        $cekslug   = PortalInfografis::whereSlug($slug)->count();
        $inputslug = $cekslug > 0 ? $slug . "-" . Helper::gencode(4) : $slug;

        // value
        $uuid    = Str::uuid();
        $path    = "infografis/" . date('Y') . "/" . $uuid;
        ...
        if ($request->hasFile('file_infografis')) {
            $img = Helper::UpInfografis($request, "file_infografis", $path);
            if ($img == "0") {
                alert()->error('Error!', 'Gagal Menyimpan Data, File Infografis Tidak Sesuai Format!');
                return back();
            }
            $value_1['url']  = $img['url'];
        ...
        // save
        $save_1 = PortalInfografis::create($value_1);
        if ($save_1) {
            // create log
            $aktifitas = [
                "tabel" => ["portal_infografis"],
                "uuid"  => [$uuid],
                "value" => [$request->judul],
            ];
            $log = [
                "apps"      => "Portal Apps",
                "subjek"    => "Menambahkan Data Infografis UUID= " . $uuid,
                "aktifitas" => $aktifitas,
                "device"    => "web",
            ];
            Helper::addToLogAktifitas($request, $log);
            alert()->success('Success', "Berhasil Menambahkan Data!");
            return redirect()->route('prt.apps.infografis.index');
        } else {
    ...
    public function edit($uuid_enc)
    {
        ...
        $data = PortalInfografis::findOrFail($uuid);
        // get kategori
        $kategoriList = PortalKategori::whereType("Infografis")->whereStatus("1")->orderBy("nama")->get();
        $title        = "Edit Data Infografis";
        $submit       = "Simpan";
        return view('pages.admin.portal_apps.infografis.create_edit', compact(
            'uuid_enc',
            'title',
    ...
    public function update(Request $request, $uuid_enc)
    {
        ...
        $request->validate([
            ...
            "post"            => "sometimes|nullable",
            "kategori"        => "required",
            "file_infografis" => "sometimes|image|mimes:png,jpg,jpeg|max:2048",
            "tanggal"         => "required",
        ]);
        ...
        if ($data->judul !== $request->judul) {
            $slug      = \Str::slug($request->judul);
            $cekslug   = PortalInfografis::where('uuid', '!=', $uuid)->whereSlug($slug)->count();
            $inputslug = $cekslug > 0 ? $slug . "-" . Helper::gencode(4) : $slug;
        } else {
            $inputslug = $data->slug;
        }
        ...
        // file_infografis
        if ($request->hasFile('file_infografis')) {
            if (! empty($data->file_infografis) && Storage::disk('public')->exists($data->file_infografis)) {
                Storage::disk('public')->delete($data->file_infografis);
                $thumbnailPath = str_replace('.', '_thumbnail.', $data->file_infografis);
        ...
        if ($save_1) {
            ...
            $log = [
                "apps"      => "Portal Apps",
                "subjek"    => "Mengubah Data Infografis UUID= " . $uuid,
                "aktifitas" => $aktifitas,
                "device"    => "web",
            ];
            Helper::addToLogAktifitas($request, $log);
    ...
    public function destroy(Request $request)
    {
        ...
        if ($data->status == "Draft" || $data->status == "Pending Review") {
            ...
            $path  = "infografis/{$tahun}/{$data->uuid}";
        ...
        if ($save_1) {
            ...
            $log = [
                "apps"      => "Portal Apps",
                "subjek"    => "Menghapus Data Infografis UUID= " . $uuid,
                "aktifitas" => $aktifitas,
                "device"    => "web",
            ];
            Helper::addToLogAktifitas($request, $log);
