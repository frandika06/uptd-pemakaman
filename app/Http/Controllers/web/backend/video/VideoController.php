...
class VideoController extends Controller
{
        ...
        if ($request->ajax()) {
            ...
            return Datatables::of($data)
                ...
                ->addColumn('judul', function ($data) {
                    ...
                    $edit_url = route('prt.apps.video.edit', $uuid_enc);

                                                                                   // Generate thumbnail URL
                    $thumbnail_url = asset('be/media/misc/image-placeholder.svg'); // Default placeholder
                    if ($data->sumber == "YouTube") {
                        $thumbnail_url = Helper::getYouTubeThumbnailUrl($data->url, "default");
                    } elseif (! empty($data->thumbnails)) {
    ...
    public function bulkDestroy(Request $request)
    {
        try {
            ...
            foreach ($uuids as $uuid) {
                try {
                    ...
                    if ($save_1) {
                        ...
                        $log = [
                            "apps"      => "Portal Apps",
                            "subjek"    => "Menghapus Data Video (Bulk): " . $data->judul . " - " . $uuid,
                            "aktifitas" => $aktifitas,
                            "device"    => "web",
                        ];
                        Helper::addToLogAktifitas($request, $log);
            ...
            $message = "Berhasil menghapus {$deletedCount} video";
            ...
            $summaryLog = [
                "apps"      => "Portal Apps",
                "subjek"    => "Bulk Delete Video - Berhasil: {$deletedCount}, Gagal: " . count($failedItems),
                "aktifitas" => [
                    "tabel"         => ["portal_video"],
            ...
            Log::error('Bulk Delete Video Error', [
