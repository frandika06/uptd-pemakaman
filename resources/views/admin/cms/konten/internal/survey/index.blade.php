@push('styles')
    <style>
        ...
        [data-kt-links-table-toolbar="selected"] {
            transition: all 0.3s ease;
            opacity: 0;
            transform: translateY(-10px);
        }

        [data-kt-links-table-toolbar="selected"]:not(.d-none) {
            opacity: 1;
...
@section('toolbar')
    <div class="d-flex flex-stack flex-row-fluid">
        ...
        <div class="d-flex align-self-center flex-center flex-shrink-0">
            ...
            <a href="{{ route('prt.apps.links.create', [$tags]) }}" class="btn btn-sm btn-primary d-flex flex-center ms-3 px-4 py-3">
                <i class="ki-outline ki-plus fs-2"></i>
                <span>Tambah Links</span>
            </a>
            {{-- end::Primary button --}}
        </div>
        {{-- end::Actions --}}
...
@section('content')
    ...
    <div class="card">
        ...
        <div class="card-header border-0 pt-6">
            ...
            <div class="card-title">
                <h4 class="card-title align-items-start flex-column">
                    <span class="card-label fw-bold fs-3 mb-1">Data Links {{ $kategori }}</span>
                </h4>
            </div>
            {{-- end::Card title --}}
            {{-- begin::Card toolbar --}}
            <div class="card-toolbar">
                ...
                <div class="d-flex align-items-center position-relative my-1">
                    ...
                    <input type="text" data-kt-links-table-filter="search" class="form-control form-control-solid w-250px ps-14" placeholder="Cari links..." />
                </div>
                {{-- end::Search --}}
            </div>
            {{-- end::Card toolbar --}}
        ...
        <div class="card-body py-4">
            {{-- begin::Bulk Actions --}}
            <div class="d-flex justify-content-between align-items-center d-none bg-light-primary rounded p-3 mb-5" data-kt-links-table-toolbar="selected">
                <div class="fw-bold text-primary">
                    <i class="ki-outline ki-check-square fs-2 me-2"></i>
                    <span data-kt-links-table-select="selected_count"></span> item dipilih
                </div>

                <div class="d-flex align-items-center gap-2">
                    {{-- begin::Bulk Status Actions --}}
                    <div class="btn-group me-2" role="group" aria-label="Status Actions">
                        <button type="button" class="btn btn-sm btn-light-success" data-kt-links-table-select="activate_selected" data-bs-toggle="tooltip"
                            title="Aktifkan yang dipilih">
                            <i class="ki-outline ki-check fs-6 me-1"></i>
                            Aktifkan
                        </button>
                        <button type="button" class="btn btn-sm btn-light-warning" data-kt-links-table-select="deactivate_selected" data-bs-toggle="tooltip"
                    ...
                    <button type="button" class="btn btn-sm btn-light-danger me-2" data-kt-links-table-select="delete_selected" data-bs-toggle="tooltip" title="Hapus yang dipilih">
                    ...
                    <button type="button" class="btn btn-sm btn-light" data-kt-links-table-select="cancel_selection" data-bs-toggle="tooltip" title="Batalkan pilihan">
...
@push('scripts')
    <script>
        ...
        // Class definition
        var KTDatatablesLinks = function() {
            // Shared variables
            var table;
            var datatable;
            ...
            var initDatatable = function() {
                ...
                datatable = $('#datatable').DataTable({
                    ...
                    "ajax": {
                        "url": "{!! route('prt.apps.links.index', [$tags]) !!}",
                        "type": 'GET',
            ...
            var exportButtons = function() {
                const documentTitle = 'Data Links {{ $kategori }}';
                    ...
                    buttons: [{
                        ...
                        },
                        {
                            ...
                            filename: function() {
                                ...
                                return `links-{{ \Str::slug($kategori) }}-${date}`;
                            },
                            exportOptions: {
                                columns: [1, 2, 3, 4, 5, 6]
                            }
                        ...
                        },
                        {
                            ...
                            customize: function(doc) {
                                doc.content[1].table.widths = ['10%', '35%', '12%', '12%', '11%', '10%', '10%'];
            ...
            var handleSearchDatatable = function() {
                const filterSearch = document.querySelector('[data-kt-links-table-filter="search"]');
                if (filterSearch) {
            ...
            var handleBulkActions = function() {
                ...
                const bulkToolbar = document.querySelector('[data-kt-links-table-toolbar="selected"]');
                const countElement = document.querySelector('[data-kt-links-table-select="selected_count"]');
            ...
            var handleEvents = function() {
                ...
                $(document).on('click', '[data-kt-links-table-select="cancel_selection"]', function() {
                ...
                $(document).on('click', '[data-kt-links-table-select="activate_selected"]', function() {
                    handleBulkStatus('1', 'mengaktifkan');
                });

                // Handle bulk status - Deactivate
                $(document).on('click', '[data-kt-links-table-select="deactivate_selected"]', function() {
                    handleBulkStatus('0', 'menonaktifkan');
                });

                // Handle bulk delete
                $(document).on('click', '[data-kt-links-table-select="delete_selected"]', function() {
                    ...
                    Swal.fire({
                        ...
                        text: `Apakah Anda yakin ingin menghapus ${selectedIds.length} links yang dipilih?`,
                        icon: "warning",
                        ...
                        if (result.isConfirmed) {
                            ...
                            $.ajax({
                                url: "{!! route('prt.apps.links.destroy.bulk', [$tags]) !!}",
                                type: 'POST',
                                data: {
                                    uuids: selectedIds,
                                    _token: "{{ csrf_token() }}"
                ...
                $(document).on('click', "[data-delete]", function() {
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            $.ajax({
                                url: "{!! route('prt.apps.links.destroy', [$tags]) !!}",
                                type: 'POST',
                                data: {
                                    uuid: uuid,
                                    _token: "{{ csrf_token() }}"
                ...
                $(document).on('click', "[data-status]", function() {
                    ...
                    $.ajax({
                        url: "{!! route('prt.apps.links.status', [$tags]) !!}",
                        type: 'POST',
                        data: {
                            uuid: uuid,
                            status: status,
            ...
            var handleBulkStatus = function(status, actionText) {
                ...
                Swal.fire({
                    ...
                    text: `Apakah Anda yakin ingin ${actionText} ${selectedIds.length} links yang dipilih?`,
                    icon: "question",
                    showCancelButton: true,
                    ...
                    if (result.isConfirmed) {
                        ...
                        $.ajax({
                            url: "{!! route('prt.apps.links.status.bulk', [$tags]) !!}",
                            type: 'POST',
                            data: {
                                uuids: selectedIds,
                                status: status,
