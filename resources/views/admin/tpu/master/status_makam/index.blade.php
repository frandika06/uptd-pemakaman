@push('styles')
    <style>
        ...
        [data-kt-status-makam-table-toolbar="selected"] {
            transition: all 0.3s ease;
            opacity: 0;
            transform: translateY(-10px);
        }

        [data-kt-status-makam-table-toolbar="selected"]:not(.d-none) {
            opacity: 1;
...
{{-- SEO::BEGIN --}}
@section('title', 'Master Status Makam')
{{-- SEO::END --}}
    ...
    <div class="d-flex flex-stack flex-row-fluid">
        ...
        <div class="d-flex flex-column flex-row-fluid">
            ...
            <ul class="breadcrumb breadcrumb-separatorless fw-semibold mb-3 fs-7">
                ...
                {{-- end::Item --}}
                {{-- begin::Item --}}
                <li class="breadcrumb-item text-gray-700">Status Makam</li>
                {{-- end::Item --}}
            </ul>
            {{-- end::Breadcrumb --}}
        </div>
        ...
        <div class="d-flex align-self-center flex-center flex-shrink-0">
            ...
            <a href="{{ route('tpu.ref-status-makam.create') }}" class="btn btn-sm btn-primary d-flex flex-center ms-3 px-4 py-3">
                <i class="ki-outline ki-plus fs-2"></i>
                <span>Tambah Status Makam</span>
            </a>
            {{-- end::Primary button --}}
        </div>
        {{-- end::Actions --}}
...
@section('content')
    ...
    <div class="card">
        ...
        <div class="card-header border-0 pt-6">
            ...
            <div class="card-title">
                <h4 class="card-title align-items-start flex-column">
                    <span class="card-label fw-bold fs-3 mb-1">Data Master Status Makam</span>
                    <span class="text-muted mt-1 fw-semibold fs-7">Menampilkan semua status makam</span>
                </h4>
            </div>
            {{-- end::Card title --}}
            {{-- begin::Card toolbar --}}
            <div class="card-toolbar">
                ...
                <div class="d-flex align-items-center position-relative my-1">
                    ...
                    <input type="text" data-kt-status-makam-table-filter="search" class="form-control form-control-solid w-250px ps-14" placeholder="Cari status makam..." />
                </div>
                {{-- end::Search --}}
            </div>
            {{-- end::Card toolbar --}}
        ...
        <div class="card-body py-4">
            {{-- begin::Bulk Actions --}}
            <div class="d-flex justify-content-between align-items-center d-none bg-light-primary rounded p-3 mb-5" data-kt-status-makam-table-toolbar="selected">
                <div class="fw-bold text-primary">
                    <i class="ki-outline ki-check-square fs-2 me-2"></i>
                    <span data-kt-status-makam-table-select="selected_count"></span> item dipilih
                </div>

                <div class="d-flex align-items-center gap-2">
                    {{-- begin::Bulk Status Actions --}}
                    <div class="btn-group me-2" role="group" aria-label="Status Actions">
                        <button type="button" class="btn btn-sm btn-light-success" data-kt-status-makam-table-select="activate_selected" data-bs-toggle="tooltip"
                            title="Aktifkan yang dipilih">
                            <i class="ki-outline ki-check fs-6 me-1"></i>
                            Aktifkan
                        </button>
                        <button type="button" class="btn btn-sm btn-light-warning" data-kt-status-makam-table-select="deactivate_selected" data-bs-toggle="tooltip"
                    ...
                    <button type="button" class="btn btn-sm btn-light-danger me-2" data-kt-status-makam-table-select="delete_selected" data-bs-toggle="tooltip"
                    ...
                    <button type="button" class="btn btn-sm btn-light" data-kt-status-makam-table-select="cancel_selection" data-bs-toggle="tooltip" title="Batalkan pilihan">
...
@push('scripts')
    <script>
        ...
        // Class definition
        var KTDatatablesMasterStatusMakam = function() {
            // Shared variables
            var table;
            var datatable;
            ...
            var initDatatable = function() {
                ...
                datatable = $('#datatable').DataTable({
                    ...
                    "ajax": {
                        "url": "{!! route('tpu.ref-status-makam.index') !!}",
                        "type": 'GET',
            ...
            var exportButtons = function() {
                const documentTitle = 'Master Data Status Makam';
                    ...
                    buttons: [{
                        ...
                        },
                        {
                            ...
                            filename: function() {
                                ...
                                return `master-status-makam-${date}`;
                            },
                            exportOptions: {
                                columns: [1, 2, 3]
                            }
            ...
            var handleSearchDatatable = function() {
                const filterSearch = document.querySelector('[data-kt-status-makam-table-filter="search"]');
                if (filterSearch) {
            ...
            var handleBulkActions = function() {
                ...
                const bulkToolbar = document.querySelector('[data-kt-status-makam-table-toolbar="selected"]');
                const countElement = document.querySelector('[data-kt-status-makam-table-select="selected_count"]');
            ...
            var handleEvents = function() {
                ...
                $(document).on('click', '[data-kt-status-makam-table-select="cancel_selection"]', function() {
                ...
                $(document).on('click', '[data-kt-status-makam-table-select="activate_selected"]', function() {
                    handleBulkStatus('1', 'mengaktifkan');
                });

                // Handle bulk status - Deactivate
                $(document).on('click', '[data-kt-status-makam-table-select="deactivate_selected"]', function() {
                    handleBulkStatus('0', 'menonaktifkan');
                });

                // Handle bulk delete
                $(document).on('click', '[data-kt-status-makam-table-select="delete_selected"]', function() {
                    ...
                    Swal.fire({
                        ...
                        text: `Apakah Anda yakin ingin menghapus ${selectedIds.length} status makam yang dipilih?`,
                        icon: "warning",
                        ...
                        if (result.isConfirmed) {
                            ...
                            $.ajax({
                                url: "{!! route('tpu.ref-status-makam.destroy.bulk') !!}",
                                type: 'POST',
                                data: {
                                    uuids: selectedIds,
                                    _token: "{{ csrf_token() }}"
                ...
                $(document).on('click', "[data-delete]", function() {
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            $.ajax({
                                url: "{!! route('tpu.ref-status-makam.destroy') !!}",
                                type: 'POST',
                                data: {
                                    uuid: uuid,
                                    _token: "{{ csrf_token() }}"
                ...
                $(document).on('click', "[data-status]", function() {
                    ...
                    $.ajax({
                        url: "{!! route('tpu.ref-status-makam.status') !!}",
                        type: 'POST',
                        data: {
                            uuid: uuid,
                            status: status,
            ...
            var handleBulkStatus = function(status, actionText) {
                ...
                Swal.fire({
                    ...
                    text: `Apakah Anda yakin ingin ${actionText} ${selectedIds.length} status makam yang dipilih?`,
                    icon: "question",
                    showCancelButton: true,
                    ...
                    if (result.isConfirmed) {
                        ...
                        $.ajax({
                            url: "{!! route('tpu.ref-status-makam.status.bulk') !!}",
                            type: 'POST',
                            data: {
                                uuids: selectedIds,
                                status: status,
        ...
        $(document).ready(function() {
            KTDatatablesMasterStatusMakam.init();
        });
    </script>
@endpush
{{-- SCRIPTS::END --}}
