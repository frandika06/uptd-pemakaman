...
class TpuRefStatusMakamController extends Controller
{
    /**
    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            // Ambil data status makam - Tidak perlu filter TPU karena ini master data
            $data = TpuRefStatusMakam::orderBy('nama', 'ASC')->get();
            ...
            return DataTables::of($data)
                ...
                ->addColumn('nama', function ($data) {
                    ...
                    $edit_url = route('tpu.ref-status-makam.edit', $uuid_enc);
                    return '
        ...
        return view('admin.tpu.master.status_makam.index');
    }
    ...
    {
        $title  = 'Tambah Data Master Status Makam';
        $submit = 'Simpan';
        return view('admin.tpu.master.status_makam.create_edit', compact(
            'title',
            'submit'
        ));
    }
    ...
    public function store(Request $request)
    {
        ...
        // Cek status makam
        $cekStatus = TpuRefStatusMakam::where('nama', $nama)->first();
        if ($cekStatus !== null) {
            alert()->error('Error!', 'Nama Status Makam Sudah Ada!');
        ...
        // Save
        $save = TpuRefStatusMakam::create($value);
        if ($save) {
            // Create log
            $aktifitas = [
                'tabel' => ['tpu_ref_status_makam'],
                'uuid'  => [$uuid],
                'value' => [$value],
            ];
            $log = [
                'apps'      => 'TPU Admin',
                'subjek'    => 'Menambahkan Data Master Status Makam: ' . $nama . ' - ' . $uuid,
                'aktifitas' => $aktifitas,
                'device'    => 'web',
            ];
            ...
            return \redirect()->route('tpu.ref-status-makam.index');
        } else {
    ...
    public function edit($uuid_enc)
    {
        ...
        $data   = TpuRefStatusMakam::findOrFail($uuid);
        $title  = 'Edit Data Master Status Makam';
        $submit = 'Simpan';
        return view('admin.tpu.master.status_makam.create_edit', compact(
            'uuid_enc',
            'title',
    ...
    public function update(Request $request, $uuid_enc)
    {
        ...
        if ($save) {
            ...
            $log = [
                'apps'      => 'TPU Admin',
                'subjek'    => 'Mengubah Data Master Status Makam: ' . $nama . ' - ' . $uuid,
                'aktifitas' => $aktifitas,
                'device'    => 'web',
            ];
            Helper::addToLogAktifitas($request, $log);
    ...
    public function destroy(Request $request)
    {
        ...
        if ($save) {
            ...
            $log = [
                'apps'      => 'TPU Admin',
                'subjek'    => 'Menghapus Data Master Status Makam: ' . $data->nama . ' - ' . $uuid,
                'aktifitas' => $aktifitas,
                'device'    => 'web',
            ];
            Helper::addToLogAktifitas($request, $log);
    ...
    public function status(Request $request)
    {
        ...
        if ($save) {
            ...
            $log = [
                'apps'      => 'TPU Admin',
                'subjek'    => 'Mengubah Status Master Status Makam: ' . $data->nama . ' - ' . $uuid,
                'aktifitas' => $aktifitas,
                'device'    => 'web',
            ];
            Helper::addToLogAktifitas($request, $log);
    ...
    public function bulkDestroy(Request $request)
    {
        try {
            ...
            foreach ($uuids as $uuid_enc) {
                try {
                    ...
                    if ($data->delete()) {
                        ...
                        $log = [
                            'apps'      => 'TPU Admin',
                            'subjek'    => 'Menghapus Data Master Status Makam (Bulk): ' . $data->nama . ' - ' . $uuid,
                            'aktifitas' => $aktifitas,
                            'device'    => 'web',
                        ];
                        Helper::addToLogAktifitas($request, $log);
            ...
            $message = 'Berhasil menghapus ' . $deletedCount . ' status makam';
            ...
            $summaryLog = [
                'apps'      => 'TPU Admin',
                'subjek'    => 'Bulk Delete Master Status Makam - Berhasil: ' . $deletedCount . ', Gagal: ' . count($failedItems),
                'aktifitas' => [
                    'tabel'         => ['tpu_ref_status_makam'],
    ...
    public function bulkStatus(Request $request)
    {
        try {
            ...
            foreach ($uuids as $uuid_enc) {
                try {
                    ...
                    if ($data->update($value)) {
                        ...
                        $log = [
                            'apps'      => 'TPU Admin',
                            'subjek'    => 'Bulk Update Status Master Status Makam: ' . $data->nama . ' - ' . $uuid,
                            'aktifitas' => $aktifitas,
                            'device'    => 'web',
                        ];
                        Helper::addToLogAktifitas($request, $log);
            ...
            $message    = 'Berhasil ' . $statusText . ' ' . $updatedCount . ' status makam';
            if (! empty($failedItems)) {
