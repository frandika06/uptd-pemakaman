@push('styles')
    <style>
        ...
        [data-kt-page-table-toolbar="selected"] {
            transition: all 0.3s ease;
            opacity: 0;
            transform: translateY(-10px);
        }

        [data-kt-page-table-toolbar="selected"]:not(.d-none) {
            opacity: 1;
    ...
    <div class="d-flex flex-stack flex-row-fluid">
        <div class="d-flex flex-column flex-row-fluid">
            ...
            <ul class="breadcrumb breadcrumb-separatorless fw-semibold mb-3 fs-7">
                ...
                <li class="breadcrumb-item text-gray-700">Pesan</li>
            </ul>
        </div>
        <div class="d-flex align-self-center flex-center flex-shrink-0">
            <div class="me-3">
                ...
                <div class="menu menu-sub menu-sub-dropdown w-250px w-md-300px" data-kt-menu="true" id="kt_menu_filter">
                    ...
                    <div class="px-7 py-5">
                        <div class="mb-10">
                            <label class="form-label fw-semibold">Status Pesan:</label>
                            <div>
                                <select class="form-select form-select-solid" name="q_status_pesan" id="q_status_pesan" data-control="select2" data-placeholder="Pilih Status"
                                    data-allow-clear="true">
                                    <option @if ($status == 'Pending') selected @endif value="Pending">Pending</option>
                                    <option @if ($status == 'Responded') selected @endif value="Responded">Responded</option>
                                </select>
                            </div>
                        </div>
...
@section('content')
    <div class="row g-5 g-xl-10 mb-5 mb-xl-10">
        @foreach (['Pending' => 'warning', 'Responded' => 'success'] as $label => $color)
            <div class="col-xxl-2 col-lg-4 col-sm-6">
                <div class="card stats-card bg-body hoverable card-xl-stretch mb-xl-8">
                    <div class="card-body">
                        <div class="text-gray-900 fw-bold fs-2 mb-2 mt-5">{{ Helper::GetStatistikByModel('Pesan', $label) }}</div>
                        ...
                        <div class="stats-icon stats-{{ \Str::slug($label) }} position-absolute top-0 end-0 mt-3 me-3">
                            <i class="ki-outline ki-{{ $label == 'Pending' ? 'message-text' : 'check-circle' }} fs-2 text-white"></i>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="card-header border-0 pt-6">
            <div class="card-title">
                <h4 class="card-title align-items-start flex-column">
                    <span class="card-label fw-bold fs-3 mb-1">Data Kotak Pesan</span>
            ...
            <div class="card-toolbar">
                <div class="d-flex align-items-center position-relative my-1">
                    ...
                    <input type="text" data-kt-page-table-filter="search" class="form-control form-control-solid w-250px ps-14" placeholder="Cari pesan..." />
                </div>
            </div>
        </div>
        <div class="card-body py-4">
            <div class="d-flex justify-content-between align-items-center d-none bg-light-primary rounded p-3 mb-5" data-kt-page-table-toolbar="selected">
                <div class="fw-bold text-primary">
                    <i class="ki-outline ki-check-square fs-2 me-2"></i>
                    <span data-kt-page-table-select="selected_count"></span> item dipilih
                </div>
                <div class="d-flex align-items-center gap-2">
                    <button type="button" class="btn btn-sm btn-light-danger me-2" data-kt-page-table-select="delete_selected" data-bs-toggle="tooltip"
                        title="Hapus yang dipilih">
                        <i class="ki-outline ki-trash fs-6 me-1"></i>
                        Hapus
                    </button>
                    <button type="button" class="btn btn-sm btn-light" data-kt-page-table-select="cancel_selection" data-bs-toggle="tooltip" title="Batalkan pilihan">
...
@push('scripts')
    ...
    <script src="{{ asset('be/plugins/custom/select2/select2.full.min.js') }}"></script>
    <script>
        "use strict";

        var KTDatatablesPesan = function() {
            var table;
            var datatable;
            ...
            var initDatatable = function() {
                datatable = $('#datatable').DataTable({
                    ...
                    order: [
                        [6, 'asc']
                    ], // Mengurutkan berdasarkan kolom tanggal (index 6)
                    language: {
                    ...
                    ajax: {
                        url: "{!! route('prt.apps.kotak.pesan.index') !!}",
                        type: 'GET',
                        data: function(data) {
                            data.filter = {
                                status: $('[name="q_status_pesan"]').val() || 'Pending'
                            };
                        },
            ...
            var exportButtons = function() {
                const documentTitle = 'Data Kotak Pesan';

                var buttons = new $.fn.dataTable.Buttons(datatable, {
                    buttons: [{
                            extend: 'copyHtml5',
                            title: function() {
                                const currentFilter = $('[name="q_status_pesan"]').val() || 'Pending';
                        ...
                        },
                        {
                            ...
                            filename: function() {
                                ...
                                const filter = ($('[name="q_status_pesan"]').val() || 'Pending').toLowerCase().replace(/\s+/g, '-');
                                return `kotak-pesan-${filter}-${date}`;
                            },
                            exportOptions: {
                                columns: [1, 2, 3, 4, 5, 6]
                            }
                        ...
                        },
                        {
                            ...
                            customize: function(doc) {
                                doc.content[1].table.widths = ['8%', '25%', '20%', '20%', '10%', '17%'];
            ...
            var handleBulkActions = function() {
                ...
                const bulkToolbar = document.querySelector('[data-kt-page-table-toolbar="selected"]');
                const countElement = document.querySelector('[data-kt-page-table-select="selected_count"]');
            ...
            var handleFilter = function() {
                $('#q_status_pesan').select2({
                    minimumResultsForSearch: Infinity
                });
                window.applyFilter = function() {
                    var selectedStatus = document.getElementById('q_status_pesan').value;
                ...
                window.resetFilter = function() {
                    document.getElementById('q_status_pesan').value = 'Pending';
                    document.getElementById('filter-text').textContent = 'Pending';
                    $('#q_status_pesan').trigger('change');
                    $('#datatable_processing').show();
                    datatable.ajax.reload(function(json) {
                        $('#datatable_processing').hide();
                        $('#titleStatus').html('Pending');
                    }, false);
                }
                $('[name="q_status_pesan"]').on('change', function() {
                    var q_status_pesan = $(this).val();
                    $('#datatable_processing').show();
                    datatable.ajax.reload(function(json) {
                        $('#datatable_processing').hide();
                        $('#titleStatus').html(q_status_pesan);
                    }, false);
                });
            }
            ...
            var handleEvents = function() {
                ...
                $(document).on('click', '[data-kt-page-table-select="cancel_selection"]', function() {
                    $('.row-checkbox').prop('checked', false);
                    $('[data-kt-check="true"]').prop('checked', false);
                    const bulkToolbar = document.querySelector('[data-kt-page-table-toolbar="selected"]');
                    bulkToolbar.classList.add('d-none');
                });
                $(document).on('click', '[data-kt-page-table-select="delete_selected"]', function() {
                    ...
                    Swal.fire({
                        ...
                        text: `Apakah Anda yakin ingin menghapus ${selectedIds.length} pesan yang dipilih?`,
                        icon: "warning",
                        ...
                        if (result.isConfirmed) {
                            ...
                            $.ajax({
                                url: "{!! route('prt.apps.kotak.pesan.destroy.bulk') !!}",
                                type: 'POST',
                                data: {
                                    uuids: selectedIds,
                                    _token: "{{ csrf_token() }}"
                ...
                $(document).on('click', '[data-kt-page-table-filter="delete_row"]', function() {
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            $.ajax({
                                url: "{!! route('prt.apps.kotak.pesan.destroy') !!}",
                                type: 'POST',
                                data: {
                                    uuid: uuid,
                                    _token: "{{ csrf_token() }}"
            ...
            return {
                init: function() {
                    ...
                    handleFilter();
                    handleEvents();
                    getStatsCounter(); // Panggil statistik saat inisialisasi
                }
            };
        }();

        $(document).ready(function() {
            KTDatatablesPesan.init();
        });
    </script>
@endpush
